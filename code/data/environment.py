...
class Environment(object):
    ...
    def standardize(self, array, entities, dims, mean=None, std=None):
        if mean is None and std is None:
            mean, std = self.get_standardize_params(entities, dims)
        elif mean is None and std is not None:
            mean, _ = self.get_standardize_params(entities, dims)
        elif mean is not None and std is None:
            _, std = self.get_standardize_params(entities, dims)
