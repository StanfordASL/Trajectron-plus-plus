...
class MultimodalGenerativeCVAE(object):
    def __init__(self,
        ...
        edge_types_str = [env.edge_type_str(edge_type) for edge_type in edge_types]
    ...
    def obtain_encoded_tensor_dict(self,
        ...
        # Encode History #
        ##################
        tensor_dict['node_history_encoded'] = self.encode_node_history(mode, node_history_st, first_history_indices, timestep)
        ...
        ##############
        if self.hyperparams['use_map_encoding']:
            cropped_maps = get_cropped_maps(world_pts=node_pos.cpu().numpy(),
                                            obs_map=scene.map.data,
                                            homography_matrix=scene.map.homography,
                                            context_size=self.hyperparams['map_context'])
            cropped_maps = torch.from_numpy(cropped_maps).to(self.device)
            cropped_maps = cropped_maps.unsqueeze(1)
            encoded_map = self.node_modules[self.node_type + '/map_encoder'](cropped_maps, node_vel_st)
            encoded_map = F.dropout(encoded_map, p=0.5,
            ...
            if self.log_writer is not None:
                self.log_writer.add_images('%s/cropped_maps' % str(self.node_type),
                                           # This is making it a 3-channel image.
                                           torch.stack([cropped_maps[:, 0]] * 3, dim=1),
                                           self.curr_iter)

                img_pts = scene.map.to_map_points(node_pos.cpu().numpy())
                ...
                self.log_writer.add_image_with_boxes('%s/cropped_locs' % str(self.node_type),
                                                     np.stack([scene.map.data] * 3, axis=0),
                                                     box_arr,
                                                     self.curr_iter)
        ...
        if mode == ModeKeys.PREDICT and self.robot_node is not None:
            ...
            # Only add this to the tiling list if we use map encoding.
            if self.hyperparams['use_map_encoding']:
                tiling_vals.append('encoded_maps')
        ...
        tensor_dict["x"] = torch.cat(concat_list,
                                     dim=1)  # [bs/nbs, 4*enc_rnn_dim + enc_rnn_dim_history + 4*enc_rnn_dim_future (only if robot_node is not None) + map_enc_output_size]
    ...
    def encode_edge(self,
        ...
        for i, timestep_in_scene in enumerate(timesteps_in_scene):  # Get neighbors for timestep in batch
            ...
            for node in connected_nodes[i]:
                ...
                rel_state[:] = node_history[i, -1].cpu()
    ...
    def p_y_xz(self, x, z_stacked, tensor_dict, mode,
        ...
        initial_state = initial_h_model(zx)#(initial_h_model(zx), initial_c_model(zx))
        ...
        if mode in [ModeKeys.TRAIN, ModeKeys.EVAL]:
            ...
            if self.hyperparams['sample_model_during_dec'] and mode == ModeKeys.TRAIN:
                ...
                for j in range(ph):
                    h_state = cell(input_, state) #, c_state
                ...
                for j in range(ph):
                    inputs = torch.cat([zx_time_tiled, dec_inputs.repeat(k, 1, 1)],
                                       dim=2)
                    h_state = cell(inputs[:, j, :], state) #, c_state
            ...
            for j in range(ph):
                ...
                gmm = GMM2D(log_pi_t, mu_t, log_sigma_t, corr_t, self.hyperparams, self.device,
                            self.hyperparams['log_sigma_min'],
                ...
                if most_likely_gmm:
                    ...
                    for i in range(gmm.mus.shape[0]):
                        gmm_i = GMM2D(log_pi_t[i], mu_t[i], log_sigma_t[i], corr_t[i], self.hyperparams,
                                      self.device,
                                      self.hyperparams['log_sigma_min'],
                                      self.hyperparams['log_sigma_max'])  # [k;bs, pred_dim]
